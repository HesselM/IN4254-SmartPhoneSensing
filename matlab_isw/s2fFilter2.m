function [m_log m_math] = s2fFilter2(m, factor)
    ...
    for i=1:maxidx-factor
        dwindow = m(i:(i+factor));
        m_math(i) = mean(dwindow);
    end
...
%{
    ...
    noise   = 25;  %1/2 seconds = all motion sizes below this value are regarded noise
    ...
    for i=2:maxidx-factor
        ...
        if (icount >= minidle) && (istart > iend) && (inmotion) && ((scount > 0) || (wcount > 0)) && (scount+wcount > noise)

            % If stepcount > walkcount: more steps detected
            % we found a step if 'motion window' < minwalk
